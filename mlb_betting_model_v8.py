# =============================================================================
# MLB BETTING MODEL - v8 (XGBoost + Recent Form + Home/Away Splits)
# =============================================================================
#
# WHAT'S NEW IN v8:
#   - XGBoost replaces Logistic/Linear Regression (smarter, learns stat interactions)
#   - Recent form weighting: last 2 weeks of games count 3x more than older games
#   - Home/Away splits: model now knows some teams play very differently at home vs away
#   - Cross-validation: accuracy is now tested across 5 splits, not just one
#   - Feature importance: shows which stats matter most after training
#
...
ML_FEATS = [
    ...
    'FIP_home','FIP_away',                        # NEW: FIP more predictive than ERA
    'K9_home','K9_away','BB9_home','BB9_away',    # NEW: strikeout/walk splits
    'sp_era_diff','sp_whip_diff',
    'sp_era_home','sp_era_away',
    'sp_k9_home','sp_k9_away',
    'home_win_pct_season',                        # NEW: home team season win%
    'away_win_pct_season',                        # NEW: away team season win%
]
...
TOT_FEATS = [
    ...
    'HR9_home','HR9_away',                        # NEW: HR/9 is key for totals
    'sp_era_home','sp_era_away',
    'sp_k9_home','sp_k9_away',
]
...
scaler_ml = scaler_tot = scaler_sp = None
...
if schedule_loaded and len(all_schedules) > 100:
    if all(c in all_schedules.columns for c in ['R','RA','Home']):
        ...
        print("\nBuilding matchup features (with home/away splits + form weighting)...")
        rows = []

        # Pre-compute per-team season win% for home/away context
        team_win_pct = {}
        for season_yr in all_schedules['Season'].unique():
            ...
            for team in ABBREV_TO_FULL.values():
                ...
                for _, row in home_g.iterrows():
                    hr = pd.to_numeric(row.get('R',np.nan),errors='coerce')
                    ar = pd.to_numeric(row.get('RA',np.nan),errors='coerce')
                    if not np.isnan(hr) and not np.isnan(ar):
                        wins += 1 if hr > ar else 0; total += 1
                for _, row in away_g.iterrows():
                    hr = pd.to_numeric(row.get('R',np.nan),errors='coerce')
                    ar = pd.to_numeric(row.get('RA',np.nan),errors='coerce')
                    if not np.isnan(hr) and not np.isnan(ar):
                        wins += 1 if ar > hr else 0; total += 1
        ...
        for _, game in all_schedules.iterrows():
            ...
            # Recent form weight: games in last RECENT_DAYS get higher weight
            ...
            rows.append({
                ...
                # Starter proxies using team ERA for training
...
def save_prediction(log_df, date, home, away, home_sp, away_sp,
    ...
    if dk_home_ml is not None and dk_away_ml is not None:
        ...
        elif win_prob < 0.5 and dk_away_ml and dk_away_ml > fair_away:
            ml_pick = away; ml_pick_odds = dk_away_ml

    total_pick = None
    if dk_total is not None:
        diff = pred_total - dk_total
        if diff >= 0.5:    total_pick = "OVER "  + str(dk_total)
        elif diff <= -0.5: total_pick = "UNDER " + str(dk_total)
...
# =============================================================================
# STEP 6: FETCH TODAY'S DRAFTKINGS ODDS
# =============================================================================
...
def predict_game(home_input, away_input, dk_home_ml=None,
    ...
    # Current season win% (used as home/away context feature)
    home_wp = team_win_pct.get((home, season), 0.500) if 'team_win_pct' in dir() else 0.500
    away_wp_season = team_win_pct.get((away, season), 0.500) if 'team_win_pct' in dir() else 0.500
    ...
    pred_total  = tot_model.predict(X_tot)[0]
    ...
    if dk_home_ml is not None:
        ...
        if win_prob >= 0.5 and dk_home_ml > fair_ml:
            ...
            print(f"  Why              : DK has them at {dk_home_ml:+d}, model fair line is {fair_ml:+d} (underpriced)")
        elif win_prob < 0.5 and dk_away_ml and dk_away_ml > fair_away:
            value_found = True
            print(f"  Value bet        : {away[:26]} {dk_away_ml:+d}  <-- MODEL SAYS BET THIS")
            print(f"  Why              : DK has them at {dk_away_ml:+d}, model fair line is {fair_away:+d} (underpriced)")
        if not value_found:
            if win_prob >= 0.5 and dk_home_ml is not None and dk_home_ml < 0:
                print(f"  Value bet        : None (model agrees {home[:22]} wins, but DK line is fair)")
            elif win_prob < 0.5 and dk_away_ml is not None and dk_away_ml < 0:
                print(f"  Value bet        : None (model agrees {away[:22]} wins, but DK line is fair)")
    ...
    print(f"  {home[:22]:<24} OBP:{h['OBP']:.3f} SLG:{h['SLG']:.3f} ERA:{h['ERA_allowed']:.2f} WHIP:{h['WHIP_allowed']:.2f} Win%:{home_wp:.3f}")
    print(f"  {away[:22]:<24} OBP:{a['OBP']:.3f} SLG:{a['SLG']:.3f} ERA:{a['ERA_allowed']:.2f} WHIP:{a['WHIP_allowed']:.2f} Win%:{away_wp_season:.3f}")
